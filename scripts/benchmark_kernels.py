...
def _bytes_model(T: int, n: int, C: int, D_out: int) -> dict[str, int]:
    ...
    # Same mandatory-bytes model used in docs/TUNING_SM120.md.
    return {
        ...
        "_fused_pre_map_bwd_fused_kernel_n4": b(T * n * C, bf16)  # x_streams
        + b(T * n, fp32)   # h_pre
        + b(T * C, bf16)   # grad_out
...
def main() -> None:
    _add_repo_to_path()
    import triton
    from flash_mhc import kernels as k
    ...
    ap.add_argument("--peak-gbps", type=float, default=1792.0, help="Peak DRAM BW for efficiency calc.")
    ...
    if use_autotune:
        ...
        if cc_major >= 12:
            # Match runtime path in ops.py for SM120.
            BLOCK_T3_FUSED = 32
            BLOCK_C3_FUSED = 256
            nw3_fused = 8
            ns3_fused = 4
    ...
    if use_autotune:
        ...
        if cc_major >= 12:
            # Match runtime path in ops.py for SM120 (RTX 5090).
            BLOCK_TF = 16
            BLOCK_CF = 256
            nw_fused = 8
            ns_fused = 2
